...
def create_worksheet_from_lesson(lesson_id: Union[int, str, ObjectId],
    ...
    if not goals_docs:
        goals_docs = list(db.lessonmappinggoals.find({"lessonId": lesson.get("lessonId")}))

    activities = list(db.lessonplanactivities.find({"lesson": lesson.get("_id")}))
    if not activities and lesson.get("lessonPlanId"):
        activities = list(db.lessonplanactivities.find({"lessonPlanId": lesson.get("lessonPlanId")}))
